...
| **Pattern 5: Kyverno Cosign Image Verification** | CI signing alone does not stop untrusted images at runtime; clusters need admission policy enforcement tied to the same signing key | Kyverno `verifyImages` in Audit and Enforce modes, using the same Cosign public key and image repository identity used by the Jenkins pipeline | [README](./kyverno-image-verification/README.md) · [Runbook](./kyverno-image-verification/kyverno-runbook.md) |
| **Pattern 6: Karpenter vs Cluster Autoscaler** | Platform teams need a practical decision framework for node autoscaling choices without confusing pod autoscaling and infrastructure autoscaling layers | Theory-only architecture comparison explaining models, trade-offs, scenario-based recommendations, sample manifests, and manual validation commands for future testing | [README](./karpenter-vs-cluster-autoscaler/README.md) · Runbook not available (theory-only) |
...
| Pattern 5: Kyverno Cosign Image Verification | [README](./kyverno-image-verification/README.md) · [Runbook](./kyverno-image-verification/kyverno-runbook.md) |
| Pattern 6: Karpenter vs Cluster Autoscaler | [README](./karpenter-vs-cluster-autoscaler/README.md) · Runbook not available (theory-only) |
